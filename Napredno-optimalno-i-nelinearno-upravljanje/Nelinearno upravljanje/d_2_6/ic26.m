...
for i=1:size(X1,1)
   for j=1:size(X2,1)
      DX1(i,j) = -X1(i,j) + a*X2(i,j) - b*X1(i,j)*X2(i,j) + X2(i,j)*X2(i,j);
      DX2(i,j) = -(a+b)*X1(i,j) + b*X1(i,j)*X1(i,j) - X1(i,j)*X2(i,j);
   end
end
...
while (max(x10, x20) < 5.5)
   [x10,x20]=ginput(1);
   out = sim('eq_2_6_model'); %Simulink model sa pocetnim uslovima u integratorima x10 i x20
